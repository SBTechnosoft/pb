...
if(isset($_POST['showtax']))
	...
	{	
		...
		for($i=0;$i<count($_POST['txtvenue']); $i++)
		{
			...
			{
				insNewEventPlac($conn,$event_id_pdtl,$eventplaces_id,$_POST['txtieqp'][$j],$_POST['txtirate'][$j],$_POST['txtiqty'][$j],$_POST['txtiamt'][$j],$_POST['txtistf'][$j],$_POST['txtivend'][$j],$_POST['txtivendprice'][$j],$_POST['txtiremark'][$j]);
			}
		}
	...
	{	
		...
		{
		?>
			<option select="<?php if($i==0){echo "selected";} ?>" value="<?php echo $data[$i]['cmp_id'];?>"><?php echo $data[$i]['cmp_name'];?></option>
		<?php	
		}
	...
	{			
		$q = mysql_query("select `price` from  equipment_mst where `eq_id` = '".$_POST['eqpid']."' ");
		$row = mysql_fetch_array($q);
		header("Content-type: text/x-json");
		echo json_encode($row);
		exit();	
