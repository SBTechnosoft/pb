...
<?php
	...
	if(isset($_POST['savepaytrn']))
	{
		if($_POST['txtpamt']!= '')
		{
			...
			if($resultShow[0]['client_charges'] >= $tot)
			{
				...
				else if($resultShow[0]['client_charges']== $tot)
				{
...
<?php
	...
	if(isset($_POST['edit']))
	{		
		$q = mysql_query("SELECT `event_id`,`event_name`,`event_ds`,`client_name`,`client_cmp`,`client_email`,`client_work_mob`,`client_home_mob`,`client_mob`,`status`,`payment_status`,`client_charges`,`client_paid_amt`,`client_discount_amt`,`from_date` FROM event_mst where `event_id` = '".$_POST['id']."' ");
